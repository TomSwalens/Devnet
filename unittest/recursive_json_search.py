...
def json_search(key,input_object):
    """     Search a key from JSON object, get nothing back if key is not found     key : "keyword" to be searched, case sensitive     input_object : JSON object to be parsed, test_data.py in this case     inner_function() is actually doing the recursive search     return a list of key:value pair     """     
    ret_val=[]     
                ...
                if k == key:                     
                    ...
                    if isinstance(v, dict): # the value is another dict so repeat                     
                        ...
                        for item in v:                         
                            ...
                            else: # Iterate a list because some APIs return JSON object in a list             for val in input_object:
